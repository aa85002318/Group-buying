      ...
      <div className="mx-auto w-full max-w-[720px] space-y-6 bg-white px-4 py-6 md:px-6 md:py-8">
        ...
        {tab === "recipes" && (
          <section className="card-surface space-y-4 p-5">
            ...
            <Button
              ...
              onClick={() => run("recipes", { ingredients })}
            >
              {loading ? "分析中…" : "AI 推薦食譜"}
            </Button>
          </section>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Input
                  ...
                  type="number"
                  value={fromServings}
                  onChange={(e) => setFromServings(e.target.value)}
                />
              </div>
              <div>
                ...
                <Input
                  ...
                  type="number"
                  value={toServings}
                  onChange={(e) => setToServings(e.target.value)}
                />
              </div>
            </div>
        ...
        {tab === "oven" && (
          <section className="card-surface space-y-4 p-5">
            ...
            <Button
              disabled={loading}
              onClick={() =>
                run("oven", { celsius: Number(celsius), from: fromMode, to: toMode })
              }
            >
              {loading ? "換算中…" : "換算溫度"}
            </Button>
        ...
        {tab === "substitute" && (
          <section className="card-surface space-y-4 p-5">
            ...
            <Button
              disabled={loading}
              onClick={() => run("substitute", { ingredient: subIngredient })}
            >
              {loading ? "分析中…" : "尋找替代"}
            </Button>
          </section>
        ...
        {tab === "failure" && (
          <section className="card-surface space-y-4 p-5">
            ...
            <Input
              ...
              placeholder="例如：蛋糕消泡、麵包不起來"
            />
            <div className="flex flex-wrap gap-2">
              {["餅乾太硬", "蛋糕消泡", "麵包不起來", "中心未熟"].map((s) => (
                <Chip key={s} tone="warning" onClick={() => setSymptom(s)}>
                  {s}
                </Chip>
              ))}
            </div>
          ...
          <div key={String(r.id)} className="rounded-2xl border border-border bg-muted/40 p-4">
            <div className="flex items-center justify-between gap-2">
              ...
              <span className="text-xs font-bold text-foreground-secondary">
                {Math.round(Number(r.matchScore) * 100)}% 匹配
              </span>
            </div>
