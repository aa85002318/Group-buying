...
export default function MemberGiftsRedeemPage() {
  ...
  const confirmRedeem = async () => {
    ...
    if ((lookup.gift_items?.length ?? 0) > 0 && !staffItemId) {
      setMessage("請先選擇贈品");
      return;
    }
    setLoading(true);
  ...
  return (
    <div className="mx-auto max-w-lg space-y-4">
      <AdminPageHeader
        title="門市核銷"
        description="掃描 QR、輸入備用碼，或以電話／會員編號人工查詢。"
      />
      <p className="text-xs text-[#687386]">
        門市作業 App 也可使用{" "}
        <Link href={APP_ROUTES.staffRedemptions} className="underline">
          /staff/redemptions
        </Link>
      </p>
      ...
      {success ? (
        <div className="space-y-2 rounded-2xl border border-[#E8E1D7] bg-white p-4">
          <h2 className="text-lg font-bold text-[#4F8A62]">核銷成功</h2>
          <p className="text-sm text-[#687386]">兌換編號：{String(success.redemption_number)}</p>
          <p className="text-sm text-[#687386]">禮品：{String(success.gift_name)}</p>
          <p className="text-sm text-[#687386]">
            會員：{(success.member as { name_masked?: string } | undefined)?.name_masked}
          </p>
          <Button
        ...
        <div className="space-y-3 rounded-2xl border border-[#E8E1D7] bg-white p-4">
          ...
          <dl className="space-y-1 text-sm text-[#687386]">
            <div className="flex justify-between">
              <dt>會員</dt>
              <dd>
                {lookup.member.name_masked}（尾碼 {lookup.member.member_tail}）
              </dd>
            </div>
            <div className="flex justify-between">
              <dt>數量</dt>
              <dd>{lookup.claim.quantity ?? 1}</dd>
            </div>
            <div className="flex justify-between">
              <dt>期限</dt>
              <dd>
                {lookup.claim.expires_at
                  ? new Date(lookup.claim.expires_at).toLocaleString("zh-TW")
                  : "—"}
              </dd>
            </div>
            <div className="flex justify-between">
              <dt>目前門市</dt>
              <dd>{lookup.store.name}</dd>
            </div>
            <div className="flex justify-between">
              <dt>狀態</dt>
              <dd>{lookup.claim.status}</dd>
            </div>
          </dl>
          {staffItems.length > 0 ? (
          ...
          {lookup.can_redeem ? (
            <Button
              ...
              disabled={loading || ((lookup.gift_items?.length ?? 0) > 0 && !staffItemId)}
      ...
      {confirmOpen ? (
        <div className="fixed inset-0 z-50 flex items-end bg-black/40 p-4 sm:items-center sm:justify-center">
          <div className="w-full max-w-md space-y-3 rounded-2xl bg-white p-5">
            ...
            <p className="text-sm text-[#687386]">
              完成後無法取消，也無法重複兌換。請確認禮品已備妥並準備交付會員。
            </p>
