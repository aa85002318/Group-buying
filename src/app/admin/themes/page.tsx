...
const emptyForm = {
  ...
  description: "",
  theme_color: "",
  status: "draft" as ThemeRow["status"],
...
export default function AdminThemesPage() {
  const { paginated, search, setSearch, page, setPage, totalPages, refresh, loading } =
    useAdminList<ThemeRow>("/api/admin/themes", "themes", ["title", "slug"]);
  ...
  const save = async () => {
    ...
    const payload = {
      ...
      mobile_cover_image_url: form.mobile_cover_image_url.trim() || null,
    ...
    const res = editing
      ? await fetch(`/api/admin/themes/${editing.id}`, {
          method: "PATCH",
  ...
  const toggleFeatured = async (row: ThemeRow) => {
  ...
  return (
    <div className="space-y-4">
      <AdminPageHeader
        title="季節主題管理"
        description="首頁「季節主題企劃」區塊內容"
        actions={
          <div className="flex flex-wrap gap-2">
            <Button onClick={openCreate}>新增主題</Button>
            <Link
              href="/admin/home"
      ...
      {showForm && (
        <div className="space-y-3 rounded-xl bg-white p-4 shadow-card">
          ...
          <Input placeholder="手機封面圖" value={form.mobile_cover_image_url} onChange={(e) => setForm({ ...form, mobile_cover_image_url: e.target.value })} />
          <Input placeholder="主題色（#hex）" value={form.theme_color} onChange={(e) => setForm({ ...form, theme_color: e.target.value })} />
          ...
          <select className="input-field" value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value as ThemeRow["status"] })}>
