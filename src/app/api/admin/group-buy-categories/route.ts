...
export async function GET() {
  ...
  if (!isSupabaseConfigured()) {
    return NextResponse.json({
      categories: [
        { id: "gbc-1", name: "食材原料", slug: "ingredients", sort_order: 10, is_active: true },
        { id: "gbc-2", name: "器具耗材", slug: "tools", sort_order: 20, is_active: true },
      ],
    });
  }
...
export async function PATCH(request: Request) {
  ...
  if (!isSupabaseConfigured()) return NextResponse.json({ category: body });

  const admin = createAdminClient();
  const { data: old } = await admin.from("group_buy_categories").select("*").eq("id", id).single();
...
export async function DELETE(request: Request) {
  ...
  const admin = createAdminClient();
  const { data: old } = await admin.from("group_buy_categories").select("*").eq("id", id).single();
  const { error: dError } = await admin.from("group_buy_categories").delete().eq("id", id);
