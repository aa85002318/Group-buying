...
export async function GET() {
  ...
  const [draft, scheduled, versions, live] = await Promise.all([
    shopLayoutVersions.getDraft(),
    shopLayoutVersions.getScheduled(),
    shopLayoutVersions.listVersions(),
...
/** Save draft only — does not affect live /shop. */
export async function PUT(request: Request) {
  ...
  const settings = mergeShopLayoutSettings(body.settings ?? body);
  const validationError = validateShopLayoutSettings(settings);
  if (validationError) {
    return NextResponse.json({ error: validationError }, { status: 400 });
  }

  const draft = await shopLayoutVersions.saveDraft(settings, {
...
export async function POST(request: Request) {
  ...
  try {
    if (action === "publish") {
      const result = await shopLayoutVersions.publishDraft({
    ...
    if (action === "schedule") {
      ...
      const scheduled = await shopLayoutVersions.scheduleDraft({
    ...
    if (action === "cancel_schedule") {
      await shopLayoutVersions.cancelSchedule(auth!.profile.id);
      return NextResponse.json({ ok: true });
    }

    if (action === "reset_draft_from_live") {
      const draft = await shopLayoutVersions.resetDraftFromLive(auth!.profile.id);
    ...
    if (action === "restore") {
      ...
      const draft = await shopLayoutVersions.restoreVersion(versionId, auth!.profile.id);
      return NextResponse.json({ draft: toLite(draft), settings: draft.snapshot });
    }
