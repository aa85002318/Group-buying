...
export function DesktopFooter() {
  ...
  return (
    <footer className="border-t border-[#F0E6C8] bg-[#FFFEFA] text-[#687386]">
      <DesktopContainer className="grid gap-6 pb-6 pt-[clamp(28px,4vw,56px)] md:grid-cols-4 md:gap-8 xl:grid-cols-5">
        <div className="space-y-3 md:col-span-1">
          ...
          {settings.tagline ? <p className="text-sm font-semibold tracking-wide text-[#153E73]">{settings.tagline}</p> : null}
          ...
          {settings.show_social ? (
            <div className="flex flex-wrap gap-3 pt-1">
              {settings.social_links.map((s) => (
                <Link key={`${s.label}-${s.href}`} href={s.href} className="text-sm font-semibold text-[#153E73] hover:text-[#F16458]">
                  {s.label}
                </Link>
              ))}
            </div>
        ...
        <div className="md:contents">
          {settings.columns.map((col, i) => {
            ...
            return (
              <div key={col.title} className="border-b border-[#E9EDF2] md:border-0">
                <button
                  type="button"
                  aria-expanded={open}
                  onClick={() => setOpenCol(open ? null : i)}
                  className="flex h-12 w-full items-center justify-between text-left text-[15px] font-bold text-[#153E73] md:pointer-events-none md:mb-3 md:h-auto md:text-sm"
                >
                  {col.title}
                ...
                <ul className={cn("space-y-3 pb-4 md:block md:space-y-2 md:pb-0", open ? "block" : "hidden")}>
                  {col.links.map((link) => (
                    <li key={`${col.title}-${link.href}`}>
                      <Link href={link.href} className="text-sm text-[#687386] transition-colors hover:text-[#153E73]">
                        {link.label}
                      </Link>
                    </li>
                  ))}
        ...
        {settings.newsletter_enabled ? (
          <div>
            ...
            <Link href={APP_ROUTES.member} className="inline-flex h-10 items-center rounded-full bg-[#153E73] px-4 text-sm font-bold text-white">
              訂閱／加入會員
            </Link>
          </div>
        ) : null}
      </DesktopContainer>
      <DesktopContainer className="flex flex-col gap-2 border-t border-[#E9EDF2] py-4 text-xs sm:flex-row sm:items-center sm:justify-between">
        ...
        {settings.bottom_links.length ? (
          <span className="flex flex-wrap gap-4">
            {settings.bottom_links.map((link) => (
              <Link key={`${link.label}-${link.href}`} href={link.href} className="hover:text-[#153E73]">
                {link.label}
              </Link>
            ))}
          </span>
