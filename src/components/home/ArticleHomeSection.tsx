    ...
    <section>
      <div className="mb-4 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <span className="h-7 w-1.5 rounded-full bg-[#4CC9A6]" />
          ...
          <Link key={a.id} href={`/articles/${a.slug}`} className="card-lift flex gap-3 overflow-hidden p-3">
            ...
            <div className="min-w-0 flex-1">
              {a.product_categories?.name && (
                <span className="text-[10px] font-bold text-[#4CC9A6]">{a.product_categories.name}</span>
              )}
              <h3 className="mt-0.5 line-clamp-2 text-sm font-bold text-coffee">{a.title}</h3>
