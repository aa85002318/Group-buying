...
import type { StepAiStructuredReply } from "@/lib/ai/recipeStepAssistant";
import type { RecipePlaybackContext } from "@/lib/recipes/media";
import type { Recipe, RecipeIngredient, RecipeTool } from "@/lib/types/database";
...
export function StoryPageAiSheet({
  open,
  onClose,
  recipe,
  context,
  ...
  const suggestions = useMemo(
    () => [
      { label: "這一步怎麼判斷成功？", prompt: "這一步怎麼判斷成功？" },
      { label: "看起來不對怎麼辦？", prompt: "看起來不對怎麼辦？" },
      { label: "常見失敗原因", prompt: "這一步常見失敗原因是什麼？" },
    ],
    []
  );
  ...
  useEffect(() => {
    ...
    setAutoAsked(false);
  }, [open, context.storyPageId, context.comparisonChoice]);
  ...
  const ask = async (question: string) => {
    ...
    try {
      const res = await fetch(`/api/recipes/${recipe.id}/ai`, {
        ...
        body: JSON.stringify({
          question: q,
          story_page_id: context.storyPageId,
          ...
          step_content: context.body ?? "",
          ai_context: context.aiContext,
          comparison_choice: context.comparisonChoice ?? null,
  ...
  useEffect(() => {
    if (!open || autoAsked || !context.initialPrompt) return;
    setAutoAsked(true);
    void ask(context.initialPrompt);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [open, autoAsked, context.initialPrompt]);
        ...
        <div className="flex items-start justify-between gap-3 border-b border-[#F2D8BF] px-4 py-3">
          <div className="min-w-0">
            ...
            <p className="mt-0.5 truncate text-xs text-foreground-secondary">
              {context.title || "故事頁"}
              {context.comparisonChoice ? ` · ${context.comparisonChoice}` : ""}
            </p>
          </div>
          <button
            type="button"
