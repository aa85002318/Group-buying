    ...
    pages: [
      ...
      },
      {
        ...
        content_config: {
          ...
          cautionEnabled: true,
          cautionTitle: "容易失敗",
          cautionItems: ["奶油不要融化", "不要攪拌過久", "糖要完全融合"],
        },
  ...
  },
  {
    ...
    pages: [
      ...
      },
      {
        ...
        layout_type: "full_bleed",
        title: "食譜挑戰",
        subtitle: "48 小時內完成即可取得徽章",
        body: "完成製作並上傳成品，即可解鎖本課徽章。",
        content_config: {
          challengeHours: 48,
          challengeBadgeLabel: "軟餅乾完成徽章",
          ctaPrimary: "繼續",
        },
      },
      {
      ...
      },
      {
        ...
        title: "分享你的作品",
        subtitle: "可略過",
        body: "完成這份食譜了嗎？歡迎上傳成品照片與製作心得，與大家交流；也可以設定為只限自己查看，保存這次的烘焙紀錄。此步驟可以略過，之後再回來分享。",
        content_config: { ctaPrimary: "結束", skipAllowed: true },
      },
    ],
  },
];
...
export async function seedDemoSmartRecipe(
  ...
  const { data: recipe, error: recipeError } = await admin
    ...
    .insert({
      ...
      slug: DEMO_SLUG,
      summary:
        "外層微酥、內部柔軟濕潤，搭配巧克力與烘烤堅果。每一步皆可觀看示範內容，並直接向老師提問。",
      cover_image: COVER_IMAGE,
      ...
      servings: "約 12 片",
      content:
        "這是 CHIMEIDIY Recipe Story Book 示範：全螢幕翻頁教材、目錄跳頁、步驟影片、向老師提問、挑戰與成品分享。",
      tips: "冷藏後再烤更不易攤平；出爐後靜置再移盤。",
      storage_method: "常溫密封約 2 天，冷藏約 5 天，冷凍約 14 天。",
      status: "published",
      published_at: now,
      seo_title: DEMO_TITLE,
      seo_description: "CHIMEIDIY 翻頁教材 DEMO：巧克力堅果軟餅乾",
      is_featured: true,
      ...
      tags: ["DEMO", "翻頁教材", "Story Book", "老師提問", "分段影片", "新手友善"],
    })
    .select("id")
    .single();
